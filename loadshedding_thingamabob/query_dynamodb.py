...
def query_recent(table_name: str, region_loadshedding: str, suffix: str, table=None):
    ...
    try:
        timestamp_recent, data_recent = database.dynamodb.get_most_recent_scraped_data(
            table, partition_key)
        logger.info(
            f'Timestamp Recent = {timestamp_recent} ({datetime.datetime.fromtimestamp(timestamp_recent).isoformat()})')
